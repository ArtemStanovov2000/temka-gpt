...
export const train = (tokensExample: number[], learningRate: number, gradientConfig: { clipValue: { min: number; max: number }; clipNorm: number; monitorFrequency: number; }) => {
  ...
  // Forward pass
  const tokenEmbeddings = inputTokens.map(token => {
    // Проверяем, существует ли эмбеддинг для токена
    if (tokenEmbedding[token]) {
      return tokenEmbedding[token];
    } else {
  ...
  // Создаем эмбеддинги для всех 64 позиций
  ...
  for (let i = 0; i < 64; i++) {
    ...
    if (tokenVec && Array.isArray(tokenVec)) {
      const combined = tokenVec.map((val, j) => {
        const posVal = posVec && posVec[j] !== undefined ? posVec[j] : 0;
        return val + posVal;
      });
